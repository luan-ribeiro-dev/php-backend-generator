...
class ControleSQL
{
	...
	{
		foreach (ControleSQL::getJsonModels() as $json) {
			ControleSQL::generateFile($json, $override);
		}
	}
	...
	private static function generateFile(array $json_object, $override)
	{
		...
		ControleSQL::checkAtributos($json_object, $class);
		ControleSQL::getAtributos($json_object, $class);
		ControleSQL::getConstrutor($json_object, $class);
		ControleSQL::getSearchObjectManageament($json_object, $class);
		ControleSQL::getDatabaseOperations($json_object, $class);
		ControleSQL::getObjectManageament($json_object, $class);
		ControleSQL::getMisc($json_object, $class);
		ControleSQL::getEncapsulations($json_object, $class);
		...
		if (count($objetos_dependentes) > 0) {
			...
			foreach ($objetos_dependentes as $atributo) {
				if ($atributo['link']['tipo'] == "objeto") {
					...
					$object = array_filter(ControleSQL::getJsonModels(), function ($object) use ($atributo) {
